...
export const getAssistantConfig = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select("-geminiApiKey -geminiApiIv")
        if (!user) {
...
`;
     ...
     const aiResponse = await generateGeminiResponse({prompt ,apikey: user.getDecryptedApiKey() , user })
